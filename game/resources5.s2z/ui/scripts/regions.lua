...
local function getLangCodes()
	if GetCvarBool('releaseStage_stable') then
		...
		return {'en', 'fr', 'de', 'br', 'es', 'ru', 'vn', 'th', 'id', 'my', 'cn', 'ua', 'tr', 'ko'}
	end
end
...
HoN_Region.regionTable = {
		...
		['international'] = {
			...
			['serverCodes'] = { 'US', 'USE', 'USW', 'EU', 'AU', 'ZAF', 'LAT', 'BR', 'RU' },																		-- Server area codes, used in public games
			['tmmRegions'] = { 'usw', 'use', 'eu', 'au', 'lat', 'br', 'ru' },																									-- Active Matchmaking regions
			...
			['cvars'] = {
				...
				{'ui_altInfoStyle', 'int', '2', true},													-- Health bar style default
		...
		['sea'] = {
			...
			['tmmAllowLeavers'] = false,
			['motdURL'] = {			-- Prefix for motd
				sbt		= 'http://sea-icb2.s3.amazonaws.com/rct/web/motd/index.html',
				rct		= 'http://sea-icb2.s3.amazonaws.com/rct/web/motd/index.html',
				retail	= 'http://sea-icb2.s3.amazonaws.com/web/motd/index.html',
			},
			['tos_paths'] = {
				en		= 'tos',
				es		= 'tos_es',
...
local function LoginSystem(isEnabled)
	if (isEnabled) then
		...
		-- GetWidget('news'):SetVAlign('center')
		...
		-- GetWidget('news'):SetVAlign('top')
		GetWidget('sysbar_center_langpicker'):SetVisible(true)
		GetWidget('sysbar_center_account_info'):SetVisible(true) -- GetWidget('sysbar_center_account_info'):SetVisible(false)
		...
		--GetWidget('main_logging_in'):SetVisible(false)
...
local function ReplaySytem(isEnabled)
	if (isEnabled) then
		GetWidget('match_replays_download_replay_btn'):SetVisible(true)
		GetWidget('sysbar_replays_button'):SetVisible(true)
	else
		GetWidget('match_replays_download_replay_btn'):SetVisible(false)
		GetWidget('sysbar_replays_button'):SetVisible(false)
	end
end
