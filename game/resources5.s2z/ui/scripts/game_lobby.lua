...
Game_Lobby.MAX_HERO_LIST_SIZE =  164
...
local function HideMenuButtonsForWalkthrough(hide)
	...
	GetWidget('midbar_button_matchmaking'):SetEnabled(enabled and not isSuspended)
...
local function UpdateHeroIcon(sourceWidget, team, param0, param1, param2, param3, param4, param5, param6, param7, param8, param9, param10, param11, param12, param13, param14, param15, param16, param17, param18, param19, param20, param21, param22, param23, param24, param25, param26, param27, param28, param29, param30, param31, param32, param33, param34, param35, param36, param37, param38, param39, param40, param41, param42, param43)
	...
	if (not AtoB(param3)) or (param2 == -5) or (AtoB(param4) and ((param2 ~= -1 and param2 ~= -3 and param2 ~= 0) or AtoB(param43) or (AtoB(param42) and AtoB(param37)))) then
...
local function UpdateHeroIconSD(sourceWidget, param0, param1, param2, param3, param4, param5, param6, param7, param8, param9, param10, param11, param12, param13, param14, param15, param16, param17, param18, param19, param20, param21, param22, param23, param24, param25, param26, param27, param28, param29, param30, param31, param32, param33, param34, param35, param36, param37, param38, param39, param40, param41, param42, param43)
	...
	if (not AtoB(param3)) or (AtoB(param4) and ((param2 ~= -1 and param2 ~= -3 and param2 ~= 0) or AtoB(param43) or (AtoB(param42) and AtoB(param37)))) then
		sourceWidget:SetRenderMode('normal')
	else
		sourceWidget:SetRenderMode('grayscale')
	end
...
local function DisplayHeroPicker(param0, param1, param2, param3, param4, param5, param6, param7, param8, param9, param10, param11, param12, param13, param14, param15, param16)
	...
	-- this was moved to gamephase, since this retriggers when a hero is picked (messing up the composition bars)
	...
	if pickMode == 'bm' or pickMode == 'km' then
		pickMode = 'ap'
	elseif pickMode == 'apg' or pickMode == 'bbg' then
		pickMode = 'gt'
	end
	...
	local mapSounds	= {
		...
		thegrimmhunt	= {
			sound			= '/shared/sounds/announcer/lobby/grimm_hunt.wav',
			announceMode	= true,
		},
		devowars	= {
...
function Game_Lobby:ToggleFilter(filter)
	...
	-- elseif (filters == 2) then
	-- 	Game_Lobby:ManuallyChangeFilterThreshold(2.0)
	-- else
	-- 	Game_Lobby:ManuallyChangeFilterThreshold(1.0)
	-- end
...
local heroSelectSlotOccupied	= {}
local heroSelectSlotColors		= {}

for i=0,9,1 do
	heroSelectSlotColors[i]		= '#FFFFFF'
	heroSelectSlotOccupied[i]	= false
end

local heroSelectTeam			= 0
local heroSelectGameMode		= ''

local function heroSelectSlotColor()
	if heroSelectGameMode == 'sd' then
		for i=0,4,1 do
			if heroSelectSlotOccupied[i + (5 * heroSelectTeam)] then
				GetWidget('sd_hero_triad_sd_'..i):SetVisible(true)
				GetWidget('sd_hero_triad_sd_'..i..'_Color'):SetColor(heroSelectSlotColors[i + (5 * heroSelectTeam)])
			else
				GetWidget('sd_hero_triad_sd_'..i):SetVisible(false)
			end
		end
	end
...
function Game_Lobby:RegisterSDTriadContainer(widget, slot)
	for i=0,1,1 do
		widget:RegisterWatch('LobbyPlayerInfo'..((i * 5) + slot), function(_, ...)
			heroSelectSlotOccupied[(i * 5) + slot]		= (arg[1] ~= '-1')
			heroSelectSlotColors[(i * 5) + slot]			= arg[3]
			heroSelectSlotColor()
		end)
	end
end

function Game_Lobby:RegisterSDTriads(widget)
	widget:RegisterWatch('HeroSelectInfo', function(_, ...)
		heroSelectTeam		= AtoN(arg[14]) - 1
		heroSelectGameMode	= arg[3]
		heroSelectSlotColor()
	end)
end
...
function Game_Lobby:InitAllyInfo(index, clientNum, playerName)
	println('InitAllyInfo() '..index..' '..clientNum..' '..playerName)
	playerName = StripClanTag(playerName)
	Game_Lobby.AllyInfo[index] = {num = clientNum, name = playerName}
end

function Game_Lobby:ToggleAllyVoiceMute(index)
	local clientNum = Game_Lobby.AllyInfo[index].num
	local playerName = Game_Lobby.AllyInfo[index].name
	local ignore = AtoB(UIManager.GetActiveInterface():UICmd([[ChatIsIgnored(']] .. playerName .. [[')]]))
	if ignore then
		UIManager.GetActiveInterface():UICmd([[ChatNotifyUnignoreFirst(']] .. playerName .. [[')]])
	else
		UIManager.GetActiveInterface():UICmd([[ToggleVoiceMute(']] .. clientNum .. [[')]])
	end
end

function Game_Lobby:ChatIgnoreChange(ignore, accountId, playerName)
	local ignore = AtoB(ignore)
	if ignore then return end
	local clientNum = nil
	for i=0,9 do
		if Game_Lobby.AllyInfo[i].name == playerName then
			clientNum = Game_Lobby.AllyInfo[i].num
			break
		end
	end
...
interface:RegisterWatch('ChatIgnoreChange', Game_Lobby.ChatIgnoreChange)
...
function Game_Lobby:SetupCampainLevel(self, index, clientNum, sameTeam, levelStr)
	local visible = clientNum and tonumber(clientNum) >= 0 and (AtoB(sameTeam) or ViewingReplay())and IsCampaignMatch()
	...
	if visible then
		...
		local text = ''
		if level > 0 then
			text = Translate('player_compaign_level_name_'..level)
		end
		self:GetWidget('HeroSelectPlayerRankLabel'..index):SetText(text)

		local colorType = ''

		if level > 17 or level < 1 then
			colorType = ''
		...
		if colorType ~= '' then
			image:SetTexture('/ui/fe2/season/icon_mini/level_'..level..'.tga')
