...
HoN_Notifications_DB = HoN_Notifications_DB or Database.New('HoN_Notifications.ldb')
...
local function GetUpgrades(userName)
	...
	if (GetChatClientInfo) and ((not nameColor) or (not accountIcon)) then
		if (HoN_Notifications.userUpgradesTable[userName]) then
			...
			if (clientInfoTable[3]) and NotEmpty(clientInfoTable[3]) then
				accountIcon = clientInfoTable[3]
			elseif (clientInfoTable[2]) and NotEmpty(clientInfoTable[2]) then
				accountIcon = clientInfoTable[2]	
			end
			if (clientInfoTable[1]) and NotEmpty(clientInfoTable[1]) then
				nameColor = clientInfoTable[1]
			end
			if (clientInfoTable[4]) and NotEmpty(clientInfoTable[4]) then
				nameGlow = clientInfoTable[4]
			else
				nameGlow = "false"
			end

			if (clientInfoTable[5] and NotEmpty(clientInfoTable[5])) then
				nameGlowColor = clientInfoTable[5]
			else
				nameGlowColor = ""
			end
			
			if ((nameColor) and NotEmpty(nameColor)) and ((accountIcon) and NotEmpty(accountIcon)) then
				...
				if ((nameGlow) and NotEmpty(nameGlow)) then 	-- nested because this is false, not empty, even if we don't have info
					HoN_Notifications.userUpgradesTable[userName].nameGlow 	= nameGlow
				end
				if (nameGlowColor and NotEmpty(nameGlowColor)) then
...
local function PlayNotificationSound(notificationType)
	if GetCvarBool('ui_not_opt_sound') then
		...
		if (soundsByType[notificationType]) and  NotEmpty(soundsByType[notificationType]) then
...
local function GetNotificationsOptionInfo(option, subOption, notificationType)
	...
	if (notificationType) then
		if (notificationType == 1) or (notificationType == 2) or (notificationType == 3) or (notificationType == 4) then
			notificationType = 1
		elseif (notificationType == 22) or (notificationType == 23) then
			notificationType = 23
		end	
	end
	...
	if (notificationType) then	
		...
		elseif (defaultNotificationOptions['ui_not_opt_' .. notificationType]) then
			if (defaultNotificationOptions['ui_not_opt_' .. notificationType][subOption]) then
				return true
			else
				return false
			end
		...
		if (defaultNotificationOptions[option]) then
			if (defaultNotificationOptions[option][subOption]) then
				return true
			else
				return false
			end
...
local function UpdateChatNotificationPopupLabels()	
	...
	if ((HoN_Notifications.notificationsCountsTable[20]) and (HoN_Notifications.notificationsCountsTable[20] > 0)) or 
		...
		--HoN_Notifications.notificationsCountsTable[17] = HoN_Notifications.notificationsCountsTable[17] or 0
	...
	for index = 1, HoN_Notifications.MAX_POPUP_NOTIFICATIONS_FRIEND, 1 do
		...
		if (targetWidget) then
			if (newNotification) then
				...
				--targetWidgetBtns 	= GetWidget('sp_notification_friend_popup_template_' .. index .. '_buttons' )
	...
	for index = 1, searchLimit, 1 do
		...
		if (targetWidget) then
			if (newNotification) then
				...
				--targetWidgetBtns 	= GetWidget('sp_notification_invite_popup_template_' .. index .. '_buttons' )
				...
				targetWidgetLabel1:SetColor(newNotification.nameColor or '#ffb300')
				if (newNotification.nameGlow and NotEmpty(newNotification.nameGlow)) then
					targetWidgetLabel1:SetGlow(AtoB(newNotification.nameGlow))
				else
					targetWidgetLabel1:SetGlow(false)
				end

				if (newNotification.nameGlowColor and NotEmpty(newNotification.nameGlowColor)) then
					targetWidgetLabel1:SetGlowColor(newNotification.nameGlowColor)
				else
					targetWidgetLabel1:SetGlowColor('')
				end
...
local function CreatePopupButtonOnclick(buttonIndex, notification)
	...
	if (buttonIndex == 1) then
		...
		elseif (notificationType == 24) then
			local param = notification.sParam1
			return function() interface:UICmd([[RejectTMMInvite(']] .. param .. [[')]]) end
		end
	elseif (buttonIndex == 2) then
		if (notificationType == 20) or (notificationType == 21) then
			local gameInfo1, gameInfo2 = notification.gameInfo1, notification.gameInfo2
			return function() interface:UICmd([[Connect(']] .. gameInfo1 .. [[', ']] .. gameInfo2 .. [[')]]) end
		elseif (notificationType == 23) then
			local param = notification.sParam1
			return function() interface:UICmd([[ChatApproveBuddy(']] .. param .. [[')']]) end
		elseif (notificationType == 24) then
			local param = notification.sParam1
			return function() HoN_Matchmaking:JoinGroup(param) end
		...
		if (targetWidget) then
			...
			elseif (buttonIndex == 2) then
				...
				if (HoN_Notifications.notificationsTable[correctedNotificationIndex].notificationType == 20) or (HoN_Notifications.notificationsTable[correctedNotificationIndex].notificationType == 21) then	-- game invite
					interface:UICmd([[Connect(']] .. HoN_Notifications.notificationsTable[correctedNotificationIndex].gameInfo1 .. [[', ']] .. HoN_Notifications.notificationsTable[correctedNotificationIndex].gameInfo2 .. [[')]])
...
local function CreateButtonOnclick(buttonIndex, notification, templateName, notificationIndex)
	...
	if (buttonIndex == 1) then
		...
		elseif (notificationType == 24) then
			local param = notification.sParam1
			return function() interface:UICmd([[RejectTMMInvite(']] .. param .. [[')]]) CloseToast() end
		end
	elseif (buttonIndex == 2) then
		if (notificationType == 20) or (notificationType == 21) or (notificationType == 15) or (notificationType == 11) then
			local gameInfo1, gameInfo2 = notification.gameInfo1, notification.gameInfo2
			return function() interface:UICmd([[Connect(']] .. gameInfo1 .. [[', ']] .. gameInfo2 .. [[')]]) CloseToast() end
		elseif (notificationType == 23) then
			local param = notification.sParam1
			return function() interface:UICmd([[ChatApproveBuddy(']] .. param .. [[')']]) CloseToast() end
		elseif (notificationType == 24) then
			local param = notification.sParam1
			return function() HoN_Matchmaking:JoinGroup(param) CloseToast() end
	...
	if (correctedNotificationIndex) and (HoN_Notifications.notificationsTable) and (HoN_Notifications.notificationsTable[correctedNotificationIndex]) then
		...
		if (targetWidget) then
			...
			elseif (buttonIndex == 3) then
				...
				-- e('buttonIndex a ', buttonIndex)
				DestroyHistoryItem(targetWidget, correctedNotificationIndex, intNotificationID)
			end
		else	
			-- e('buttonIndex b ', buttonIndex)
			DestroyHistoryItem(targetWidget, correctedNotificationIndex, intNotificationID)
		end
	else
		-- e('buttonIndex c', buttonIndex)
		DestroyHistoryItem(targetWidget, correctedNotificationIndex, intNotificationID)
	end
...
local function UpdateNotificationHistory(offset)
	...
	for slotIndex = 1, HoN_Notifications.visibleHistorySlots, 1 do
		...
		if (HoN_Notifications.autoCompleteString) and (string.len(HoN_Notifications.autoCompleteString) >= 1 ) then
			...
			-- GetWidget('sp_notification_history_template_' .. slotIndex .. '_label_1'):FadeOut(50)
			-- GetWidget('sp_notification_history_template_' .. slotIndex .. '_label_1'):Sleep(50, function()
				-- GetWidget('sp_notification_history_template_' .. slotIndex .. '_label_1'):FadeIn(50)
			-- end)
			...
			-- GetWidget('sp_notification_history_template_' .. slotIndex .. '_label_2'):FadeOut(50)
			-- GetWidget('sp_notification_history_template_' .. slotIndex .. '_label_2'):Sleep(50, function()
				-- GetWidget('sp_notification_history_template_' .. slotIndex .. '_label_2'):FadeIn(150)
			-- end)
			GetWidget('sp_notification_history_template_' .. slotIndex .. '_label_2'):SetText((FormatStringNewline(label_2 or '?')))
			
			-- GetWidget('sp_notification_history_template_' .. slotIndex .. '_label_3'):FadeOut(50)
			-- GetWidget('sp_notification_history_template_' .. slotIndex .. '_label_3'):Sleep(50, function()
				-- GetWidget('sp_notification_history_template_' .. slotIndex .. '_label_3'):FadeIn(150)
			-- end)
			GetWidget('sp_notification_history_template_' .. slotIndex .. '_label_3'):SetText(targetNotficiation.notificationTime or '?')
			
			-- GetWidget('sp_notification_history_template_' .. slotIndex .. '_icon'):SetVisible(false)
			-- GetWidget('sp_notification_history_template_' .. slotIndex .. '_icon'):Sleep(50, function()
				-- GetWidget('sp_notification_history_template_' .. slotIndex .. '_icon'):FadeIn(150)
			-- end)
...
function HoN_Notifications:UpdateUserScroller(offset)
	...
	if (HoN_Notifications.historySize > HoN_Notifications.visibleHistorySlots ) then	
		...
		if (offset) and (currentValue ~= (HoN_Notifications.visibleHistorySlots + offset)) then
			--HoN_Notifications.lastUserOffset = currentValue - HoN_Notifications.visibleHistorySlots
...
local function InstantiateToastNotification(spawnWidget, targetInterface, slotIndex, targetNotficiation)
	...
	if (not targetNotficiation) or (not targetWidget) then
		-- e('InstantiateToastNotification failed')
		-- e('targetNotficiation', targetNotficiation)
		-- e('targetWidget', targetWidget)
		-- e('spawnWidget', spawnWidget)
		-- e('targetInterface', targetInterface)
			...
			for toastIndex = 1, 3, 1 do			
				...
				if toastWidgetTable.isAvailable and (not toastWidgetTable.isClosing) and (HoN_Notifications.toastTable[1]) then
					if ( GetNotificationsOptionInfo(nil, 2, HoN_Notifications.toastTable[1].notificationType) ) then
						...
						HoN_Notifications.toastWidgetActive = HoN_Notifications.toastWidgetActive + 1
						InstantiateToastNotification(GetWidget('notification_toast_main', 'main'), 'main', toastIndex, tempToastTable)
...
function HoN_Notifications:ChatNotification(notificationSource, skipVisiblity, self, sParam1, sParam2, notificationType, notifyTranslateString, genericType, actionTemplate, notificationTime, extNotficationID, intNotificationID, sParam3, 
	...
	if (skipVisiblity) or (UIManager.GetInterface('main'):IsVisible()) then
		...
		HoN_Notifications.currentNotification = Max(HoN_Notifications.currentNotification, tonumber(intNotificationID))

		local nameColor, accountIcon, nameGlow
		if (gameInfo3) and NotEmpty(gameInfo3) then
			nameColor, accountIcon, nameGlow = GetUpgrades(gameInfo3)
		else
			nameColor, accountIcon, nameGlow = GetUpgrades(sParam1)
		end			
		
		local newNotification = {
			notificationSource = notificationSource or '',
			sParam1 = sParam1 or '',
			sParam2 = sParam2 or '',
			sParam3 = sParam3 or '',
			...
			intNotificationID = tonumber(intNotificationID) or 0,
		...
		if (HoN_Notifications.notificationsTable[tonumber(intNotificationID)]) then
			showToast = false
		end
		...
		if (HoN_Notifications.historySize <= MAXIMUM_NOTIFICATIONS) and ( GetNotificationsOptionInfo(nil, 3, newNotification.notificationType) ) then
			...
			HoN_Notifications.notificationsTable[tonumber(intNotificationID)] =  newNotification
		end
		...
		if GetCvarBool('ui_dev_notifications') then
			println('^g intNotificationID = ' .. tostring(intNotificationID) )
...
interface:RegisterWatch('ChatNotificationBuddy', 		function(...) 	HoN_Notifications:ChatNotification(false, 0, ...) end)	
interface:RegisterWatch('ChatNotificationClan', 		function(...) 	HoN_Notifications:ChatNotification(false, 1, ...) end)	
interface:RegisterWatch('ChatNotificationMessage', 		function(...) 	HoN_Notifications:ChatNotification(false, 2, ...) end)	
interface:RegisterWatch('ChatNotificationInvite', 		function(...) 	HoN_Notifications:ChatNotification(false, 3, ...) end)	
interface:RegisterWatch('ChatNotificationGroupInvite', 	function(...) 	HoN_Notifications:ChatNotification(false, 4, ...) end)	


local function ChatNotificationHistoryPerformCMD (self, uiCommand) 	-- ClearItems();
	--interface:UICmd("UICmd("..uiCommand..")")
...
function HoN_Notifications.InteractNotificationOption(sourceWidget, isClick, option, subOption, currentState)
	...
	if (GetCvarBool(tostring(option) .. '_' .. tostring(subOption), true) == nil) then
		SetSave(tostring(option) .. '_' .. tostring(subOption),  GetNotificationsOptionInfo(tostring(option), (subOption), nil), 'bool')	
	end

	if (isClick) then
		SetSave(tostring(option) .. '_' .. tostring(subOption),  (not GetCvarBool(tostring(option) .. '_' .. tostring(subOption))), 'bool')		
	end
	
	if GetCvarBool(tostring(option) .. '_' .. tostring(subOption)) then
...
function HoN_Notifications.GamePhase(sourceWidget, gamePhase)
	...
	if ((HoN_Notifications.notificationsCountsTable[20] and HoN_Notifications.notificationsCountsTable[20] > 0) or (HoN_Notifications.notificationsCountsTable[24] and HoN_Notifications.notificationsCountsTable[24] > 0)) and (tonumber(gamePhase) >= 1) then
		for i, v in pairs(HoN_Notifications.notificationsTable) do
